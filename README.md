...
3. If a match is found: Express halts the request immediately with **HTTP 400 Bad Request**.
4. If no conflict is found: The appointment is safely saved with `status: "Booked"`.
5. When rescheduling, the same query is executed while excluding the current appointment ID (`{ _id: { $ne: apptId } }`).
